...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int t = 1; t <= T; t++) {
            int N = nextInt();
            int[] inputStudentSeq = new int[N+1];
            for (int i = 0; i < N; i++)
                inputStudentSeq[i+1] = nextInt();
            ...
            for (int start = 1; start <= N; start++) {
                ...
                while (!seen.containsKey(cur)) {
                    if (!seen.containsKey(inputStudentSeq[cur]))
                        tails[cur] = Math.max(tails[cur], pos);

                    seen.put(cur, pos);
                    cur = inputStudentSeq[cur];
                    pos += 1;
                }
            ...
            int links = 0;
            for (int start = 1; start <= N; start++) {
                if (inputStudentSeq[start] < start) continue;
                if (inputStudentSeq[inputStudentSeq[start]] != start) continue;
                links += tails[start] + tails[inputStudentSeq[start]];
            }
